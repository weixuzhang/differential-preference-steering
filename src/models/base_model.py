...
class BaseModel(ABC):
    def __init__(
        ...
        offline = _is_offline()
        model_path = _cfg_get(model_configs.configs, "model_name_or_path")
        if model_path is None:
            raise ValueError("Missing model_name_or_path in model configs")
        try:
        ...
        self.max_seq_len = _cfg_get(model_configs.configs, "max_seq_len")
        self.max_new_tokens = _cfg_get(model_configs.configs, "max_new_tokens")
            ...
            if use_chat_template:
                ...
                if type(inputs) == list:
                    if "mistral" in self.model_configs.name.lower():
                        if use_system_prompt and len(inputs) > 1:
                            ...
                            # Avoid stripping the only user message; Mistral can't take a lone system prompt.
                            system_prompt = ""
