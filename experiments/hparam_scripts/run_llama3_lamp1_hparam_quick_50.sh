...
for num_heads in "${HEAD_COUNTS[@]}"; do
  ...
  run_dir="${ROOT}/dps/outputs/hparam/quick/heads/h${num_heads}"
...
mkdir -p "${ROOT}/dps/outputs/hparam/quick/gamma/adaptive"
python "${ROOT}/dps/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --scale_alpha \
  --num_samples "${NUM_SAMPLES}" \
  --run_dir "${ROOT}/dps/outputs/hparam/quick/gamma/adaptive"

FIXED_ALPHA=0.5
mkdir -p "${ROOT}/dps/outputs/hparam/quick/gamma/fixed_alpha_${FIXED_ALPHA}"
python "${ROOT}/dps/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --alpha "${FIXED_ALPHA}" \
  --num_samples "${NUM_SAMPLES}" \
  --run_dir "${ROOT}/dps/outputs/hparam/quick/gamma/fixed_alpha_${FIXED_ALPHA}"
...
for target_group in "${GROUP_SIZES[@]}"; do
  ...
  run_dir_g="${ROOT}/dps/outputs/hparam/quick/groupsize/g${target_group}"
...
mkdir -p "${ROOT}/dps/outputs/hparam/quick/ablation/random_heads"
python "${ROOT}/dps/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --temperature 1.0 \
  --num_samples "${NUM_SAMPLES}" \
  --run_dir "${ROOT}/dps/outputs/hparam/quick/ablation/random_heads"
...
mkdir -p "${ROOT}/dps/outputs/hparam/quick/ablation/random_mask"
python "${ROOT}/dps/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --temperature 1.0 \
  --num_samples "${NUM_SAMPLES}" \
  --run_dir "${ROOT}/dps/outputs/hparam/quick/ablation/random_mask"
