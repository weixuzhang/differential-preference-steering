...
for num_heads in "${HEAD_COUNTS[@]}"; do
  ...
  run_dir="${ROOT}/decore/outputs/hparam/quick/heads/h${num_heads}"
...
head_dir_40="${ROOT}/preference_head/cluster_heads/${task_slug}_k${K}_${model_slug}_quick50_h40"
detect_heads "${head_dir_40}" "${TOP_PERCENT_40}"
mkdir -p "${ROOT}/decore/outputs/hparam/quick/gamma/adaptive"
python "${ROOT}/decore/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --scale_alpha \
  --num_samples "${NUM_SAMPLES}" \
  --run_dir "${ROOT}/decore/outputs/hparam/quick/gamma/adaptive"

FIXED_ALPHA=0.5
mkdir -p "${ROOT}/decore/outputs/hparam/quick/gamma/fixed_alpha_${FIXED_ALPHA}"
python "${ROOT}/decore/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --alpha "${FIXED_ALPHA}" \
  --num_samples "${NUM_SAMPLES}" \
  --run_dir "${ROOT}/decore/outputs/hparam/quick/gamma/fixed_alpha_${FIXED_ALPHA}"

# 3) Group-size sweep (quick subset)
GROUP_SIZES=(50 100)
for target_group in "${GROUP_SIZES[@]}"; do
  K_g=$(python "${ROOT}/preference_head/compute_k.py" --task "${TASK}" --split dev --target_group "${target_group}")
  if [ -z "${K_g}" ]; then
    echo "Failed to compute K for ${TASK} (target_group=${target_group})"
    continue
  fi
  cluster_dir_g="${ROOT}/preference_head/cluster_runs/${task_slug}_k${K_g}"
  emb_file_g="${cluster_dir_g}/embeddings_dev.npy"
  head_dir_g="${ROOT}/preference_head/cluster_heads/${task_slug}_k${K_g}_${model_slug}_quick50"
  run_dir_g="${ROOT}/decore/outputs/hparam/quick/groupsize/g${target_group}"
...
head_dir_ablate="${ROOT}/preference_head/cluster_heads/${task_slug}_k${K}_${model_slug}_quick50_h40"
...
mkdir -p "${ROOT}/decore/outputs/hparam/quick/ablation/random_heads"
python "${ROOT}/decore/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --temperature 1.0 \
  --num_samples "${NUM_SAMPLES}" \
  --run_dir "${ROOT}/decore/outputs/hparam/quick/ablation/random_heads"
...
mkdir -p "${ROOT}/decore/outputs/hparam/quick/ablation/random_mask"
python "${ROOT}/decore/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --temperature 1.0 \
  --num_samples "${NUM_SAMPLES}" \
  --run_dir "${ROOT}/decore/outputs/hparam/quick/ablation/random_mask"
