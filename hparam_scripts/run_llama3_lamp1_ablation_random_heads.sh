...
mkdir -p "${ROOT}/decore/outputs/hparam/ablation/random_heads"
python "${ROOT}/decore/scripts/run_weighted_dps.py" \
  ...
  --routing soft \
  --temperature 1.0 \
  --run_dir "${ROOT}/decore/outputs/hparam/ablation/random_heads"
...
mkdir -p "${ROOT}/decore/outputs/hparam/ablation/random_mask"
python "${ROOT}/decore/scripts/run_weighted_dps.py" \
  --task "${TASK_DECODER}" \
  --model_path "${MODEL_PATH}" \
  --model_name "${MODEL_NAME}" \
  ...
  --routing soft \
  --temperature 1.0 \
  --run_dir "${ROOT}/decore/outputs/hparam/ablation/random_mask"
