...
PY
  ...
  echo "========================================="
  echo "Ablation (full dev) | ${TASK} | k=${K}"
  ...
  mkdir -p "${ROOT}/decore/outputs/hparam/ablation_full/${task_slug}/random_heads"
  python "${ROOT}/decore/scripts/run_weighted_dps.py" \
    ...
    --routing soft \
    --temperature 1.0 \
    --run_dir "${ROOT}/decore/outputs/hparam/ablation_full/${task_slug}/random_heads"
  ...
  mkdir -p "${ROOT}/decore/outputs/hparam/ablation_full/${task_slug}/random_mask"
  python "${ROOT}/decore/scripts/run_weighted_dps.py" \
    --task "${TASK_DECODER}" \
    --model_path "${MODEL_PATH}" \
    --model_name "${MODEL_NAME}" \
    ...
    --routing soft \
    --temperature 1.0 \
    --run_dir "${ROOT}/decore/outputs/hparam/ablation_full/${task_slug}/random_mask"
done
