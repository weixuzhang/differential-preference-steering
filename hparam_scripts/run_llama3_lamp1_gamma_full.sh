...
if [ "$#" -gt 0 ]; then
  ...
  TASKS=("LaMP-2" "LaMP-3" "LaMP-4" "LaMP-5" "LaMP-7")
fi
TARGET_GROUP=100
...
base = "/scratch/weixuz/preference_head/cluster_runs"
...
PY
  ...
  echo "========================================="
  echo "Gamma sweep (full dev) | ${TASK} | k=${K}"
  echo "Model: ${MODEL_NAME}"
  echo "GPU: ${GPU_ID}"
  echo "Heads: ${head_dir}"
  echo "========================================="

  mkdir -p "${ROOT}/decore/outputs/hparam/gamma_full/${task_slug}/adaptive"
  python "${ROOT}/decore/scripts/run_weighted_dps.py" \
    ...
    --temperature 1.0 \
    --scale_alpha \
    --run_dir "${ROOT}/decore/outputs/hparam/gamma_full/${task_slug}/adaptive"

  mkdir -p "${ROOT}/decore/outputs/hparam/gamma_full/${task_slug}/fixed_alpha_${FIXED_ALPHA}"
  python "${ROOT}/decore/scripts/run_weighted_dps.py" \
    --task "${TASK_DECODER}" \
    --model_path "${MODEL_PATH}" \
    --model_name "${MODEL_NAME}" \
    ...
    --temperature 1.0 \
    --alpha "${FIXED_ALPHA}" \
    --run_dir "${ROOT}/decore/outputs/hparam/gamma_full/${task_slug}/fixed_alpha_${FIXED_ALPHA}"
done
